...
public class WoollyWorkshopMenu extends AbstractContainerMenu {
    public final WoollyWorkshopBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public WoollyWorkshopMenu(int pId, Inventory pInventory, FriendlyByteBuf pBuf) {
        this(pId, pInventory, pInventory.player.level.getBlockEntity(pBuf.readBlockPos()), new SimpleContainerData(2));
    }

    public WoollyWorkshopMenu(int pId, Inventory pInventory, BlockEntity pBlockEntity, ContainerData pData) {
        super(WoollyWondersMenus.WOOLLY_WORKSHOP_MENU.get(), pId);
        checkContainerSize(pInventory, 3);
        blockEntity = (WoollyWorkshopBlockEntity) pBlockEntity;
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            this.addSlot(new SlotItemHandler(handler, 0, 38, 34));
            this.addSlot(new SlotItemHandler(handler, 1, 71, 34));
            this.addSlot(new SlotItemHandler(handler, 2, 121, 34));
        });
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, WoollyWondersBlocks.WOOLLY_WORKSHOP.get());
    }
